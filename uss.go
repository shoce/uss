...
func print() {
	...
	cpupercents, err := pscpu.Percent(PollInterval, false)
	if err != nil {
	...
	fmt.Printf(
		"@%3d\t%s\tcpu%s mem%s swap%s disk%s\n",
		tb, Hostname, cpugauge, memgauge, swapgauge, diskgauge,
	)
}
...
func main() {
	...
	if len(Hostname) > 6 {
		Hostname = Hostname[:3] + "~" + Hostname[len(Hostname)-2:]
	}
